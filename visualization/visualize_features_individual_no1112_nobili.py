...
删除了特征11-12（接触边界比例），只保留15个特征：
- 特征1-10: 基础比例和形态特征
- 特征13-17: 空间分布特征

使用方法：
python visualize_features_individual.py
"""
...
# 输出根目录
OUTPUT_ROOT = "feature_visualizations_individual"

# ============================================================================
# 独立特征可视化类
# ============================================================================

class IndividualFeatureVisualizer:
    def __init__(self, xla_path, yxa_path, output_root):
        ...
        self.feature_folders = {
            ...
            'f04_dcr': '04_SCC最大连通域占比DCR',
            'f05_fragmentation': '05_SCC碎片化指数',
            'f06_shape_factor': '06_SCC形状因子',
            'f07_perimeter_area_ratio': '07_SCC周长面积比',
            'f08_num_components': '08_SCC连通域数量',
            'f09_largest_area': '09_SCC最大连通域面积',
            'f10_total_area': '10_SCC总面积',
    ...
    def load_original_masks(self, sample_name):
        """从原始mask文件加载"""
        ...
        yxa_binary = (yxa_img > 0).astype(np.uint8)
        xla_binary = (xla_img > 0).astype(np.uint8)
        
        return xla_binary, yxa_binary  # SCC, ADC
    
    def calculate_all_features(self, scc_mask, adc_mask):
        """计算所有特征值"""
        features = {}
        
        total_pixels = scc_mask.size
        adc_pixels = np.sum(adc_mask)
        scc_pixels = np.sum(scc_mask)
        
        # 基础比例（特征1-3）
        features['adc_ratio'] = adc_pixels / total_pixels
        features['scc_ratio'] = scc_pixels / total_pixels
    ...
    def visualize_f04_dcr(self, sample_name, scc_mask, features):
        if features['dcr'] == 0:
            return None
        ...
        plt.tight_layout()
        output_file = os.path.join(self.output_root, self.feature_folders['f04_dcr'], f'{sample_name}_DCR{features["dcr"]:.10f}.jpg')
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        plt.close()
        return output_file
    
    def visualize_f05_fragmentation(self, sample_name, scc_mask, features):
        ...
        plt.tight_layout()
        output_file = os.path.join(self.output_root, self.feature_folders['f05_fragmentation'], f'{sample_name}_FRAG{features["fragmentation"]:.6f}.jpg')
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        plt.close()
        return output_file
    ...
    # 特征6: 形状因子
    # ========================================================================
    def visualize_f06_shape_factor(self, sample_name, scc_mask, features):
        ...
        plt.tight_layout()
        output_file = os.path.join(self.output_root, self.feature_folders['f06_shape_factor'], f'{sample_name}_SHAPE{features["shape_factor"]:.6f}.jpg')
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        plt.close()
        return output_file
    ...
    # 特征7: 周长面积比
    # ========================================================================
    def visualize_f07_perimeter_area_ratio(self, sample_name, scc_mask, features):
        ...
        plt.tight_layout()
        output_file = os.path.join(self.output_root, self.feature_folders['f07_perimeter_area_ratio'], f'{sample_name}_PERIM{features["perimeter_area_ratio"]:.6f}.jpg')
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        plt.close()
        return output_file
    ...
    # 特征8: 连通域数量
    # ========================================================================
    def visualize_f08_num_components(self, sample_name, scc_mask, features):
        ...
        plt.tight_layout()
        output_file = os.path.join(self.output_root, self.feature_folders['f08_num_components'], f'{sample_name}_COMP{features["num_components"]}.jpg')
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        plt.close()
        return output_file
    ...
    # 特征9: 最大连通域面积
    # ========================================================================
    def visualize_f09_largest_area(self, sample_name, scc_mask, features):
        ...
        plt.tight_layout()
        output_file = os.path.join(self.output_root, self.feature_folders['f09_largest_area'], f'{sample_name}_LARGEST{features["largest_area"]}.jpg')
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        plt.close()
        return output_file
    ...
    # 特征10: SCC总面积
    # ========================================================================
    def visualize_f10_total_area(self, sample_name, scc_mask, features):
        ...
        plt.tight_layout()
        output_file = os.path.join(self.output_root, self.feature_folders['f10_total_area'], f'{sample_name}_TOTAL{features["total_area"]}.jpg')
        plt.savefig(output_file, dpi=150, bbox_inches='tight')
        plt.close()
        return output_file
    ...
    def process_all_samples(self, max_samples=None):
        ...
        success_count = 0
        fail_count = 0
        all_samplesnames= ['1504759C2','1904016A5','2016337A5','2401573C8','1831987A5','2307414A4','2314986A5','2348033C6','2349695C1','2347312C1','2346313A1','2100776A6','2117831A4','2330172A1','2337704A4','2345069A1','1917431A2','2002208A2','1956663C1']
        
        for i, sample_name in enumerate(common_samples, 1):
            if sample_name.split('_')[0] not in all_samplesnames:
                continue
            print(f"\n[{i}/{len(common_samples)}] 处理样本: {sample_name}")
            
            try:
                scc_mask, adc_mask = self.load_original_masks(sample_name)
                ...
                features = self.calculate_all_features(scc_mask, adc_mask)
                ...
                self.visualize_f04_dcr(sample_name, scc_mask, features)
                self.visualize_f05_fragmentation(sample_name, scc_mask, features)
                self.visualize_f06_shape_factor(sample_name, scc_mask, features)
                self.visualize_f07_perimeter_area_ratio(sample_name, scc_mask, features)
                self.visualize_f08_num_components(sample_name, scc_mask, features)
                self.visualize_f09_largest_area(sample_name, scc_mask, features)
                self.visualize_f10_total_area(sample_name, scc_mask, features)
...
def main():
    """主函数"""
    print("="*60)
    print("癌症特征独立可视化工具（简化版）")
    print("删除了特征11-12（接触边界比例），保留15个特征")
    print("="*60)
    
    visualizer = IndividualFeatureVisualizer(
        xla_path=XLA_PATH,
        yxa_path=YXA_PATH,
